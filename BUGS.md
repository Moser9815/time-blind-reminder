...
- **Severity**: high (silently violated PRD Principle 8 — "reserve red for one thing")
...
- **Fix (applied)**: Replaced all 7 `#6B645A` instances in `eink-calendar/ui/index.html` with `#4A4540` (RGB-distance 73 to ink vs 112 to red). Also updated `.claude/hooks/check-palette.sh` allowlist + error message, `.claude/agents/ui-renderer-reviewer.md` rubric, `eink-calendar/docs/UI_DESIGN.md` type scale, and the `CLAUDE.md` palette pattern.
- **Verified**: render shows ink (10,282) > red (9,844), inverted from pre-fix (8,839 ink vs 10,759 red). Visual confirmation in `/tmp/postfix-review.png` — labels, hour grid, date, subtitles all read as ink; only the 92px countdown numeral, "now" line, and "now" tag are red.
